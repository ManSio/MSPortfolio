...
**Fix:** (1) `scripts/bench-claims.ts` + `pnpm bench`: прогон парафраз-набора (8 true + 3 негативных) через v1 всегда + LLM-руку при наличии ключа; пишет закоммиченный `public/benchmarks.json` (SSOT: мутационный тест тянется из experiments.json#exp-8, не хардкодится). **Живой замер: llm 88% (7/8), 0 false-acceptance, p50=1578ms p95=2237ms — воспроизвёл exp-12.** (2) `BenchmarksPanel.tsx` в секции Metrics — карточки: claim verification, mutation 8%→100%, concurrency 8/8, у каждой команда воспроизведения. (3) Уборка: Terminal-секция и компонент удалены (клише); симулятор подписан «interactive model — not a production measurement». (4) Тест целостности benchmarks.json в lab.test.ts (DoD-гейты: recall ≥80%, FA 0, p95<3s). Счётчики тестов: 92/92, docs синхронизированы.
**Guard:** числа бенчмарка — с командами и source-ссылками; benchmarks.json обновляется только через pnpm bench (не вручную); тест не даст снапшоту устареть ниже гейтов DoD.
**Pattern:** NEW
